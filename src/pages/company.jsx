...
export default function Company({ setNavbarVisible }) {
  ...
  const historyRef = useRef(null); // history 섹션의 ref
  const mainRef = useRef(null); // 내부 스크롤 컨테이너 ref

  useEffect(() => {
    const main = mainRef.current;
    if (!main || !historyRef.current) return;

    // 현재 inline 스타일 백업 (Tailwind 클래스는 그대로 유지)
    const prevSnap = main.style.scrollSnapType;

    const observer = new IntersectionObserver(
      ([entry]) => {
        // history 섹션이 뷰포트(=main 스크롤 컨테이너) 안에 들어왔을 때
        // 1) Navbar 숨김/표시
        setNavbarVisible(!entry.isIntersecting);

        // 2) 스냅 잠금/해제
        if (entry.isIntersecting) {
          // ✅ 스냅 '잠금': NavBar가 복원 시도해도 무시되게 함
          main.dataset.snapLock = "1";
          main.style.scrollSnapType = "none"; // inline이 Tailwind보다 우선
        } else {
          // ✅ 잠금 해제 + 원복
          delete main.dataset.snapLock;
          main.style.scrollSnapType = prevSnap; // 빈 문자열이면 Tailwind의 md:snap-y가 다시 적용
        }
      },
      {
        root: main, // 내부 스크롤 컨테이너 기준
        threshold: 0.5,
      }
    );
  ...
  return (
    ...
    >
      ...
      {/* 바깥 래퍼만 id="history"를 갖고, 스냅은 비활성(snap-none) */}
              ...
              <div className="flex flex-col gap-5 max-w-md mx-auto">
                <div className="flex flex-col md:flex-row items-center md:items-start justify-center md:justify-start space-y-2 md:space-y-0 md:space-x-4 text-center md:text-left">
                  <img
                    ...
                    alt="BAU 로고 부분"
                    className="h-7 object-contain mt-1 shrink-0 drop-shadow-sm"
                  />
